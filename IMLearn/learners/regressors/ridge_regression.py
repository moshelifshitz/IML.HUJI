...
class RidgeRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        u, singular_values, v = np.linalg.svd(X,full_matrices=False)
        temp = singular_values ** 2 + self.lam_
        sigma_lamda_diag = singular_values / temp
        sigma_lamda = np.diag(sigma_lamda_diag)
        self.coefs_ = v.T @ sigma_lamda @ u.T @ y
